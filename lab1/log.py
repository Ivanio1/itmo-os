...
def parse_and_display_cpu(path, output):
    ...
    wait_y= np.array([data[6] for data in data_cpu["data"]])
    ...
    axs[2].plot(usr_x, wait_y, color="red")
    axs[2].set_xlim(0, l)
    axs[2].grid()
    axs[2].set_ylabel(data_cpu["header"][6])
    axs[2].set_xlabel("seconds")

    fig.tight_layout()
    plt.savefig(os.path.join(output, "logs-pics/fig-cpu.png"))
    plt.clf()
    ...
    ax.legend()
    plt.savefig(os.path.join(output, "logs-pics/fig-cpu-1.png"))
    plt.clf()
    ...
    fig.tight_layout()
    plt.savefig(os.path.join(output, "logs-pics/fig-mem.png"))
    plt.clf()
    return data_cpu
...
def parse_and_display_io(path, output):
    ...
    fig.tight_layout()
    plt.savefig(os.path.join(output, "logs-pics/fig-io.png"))
    plt.clf()
    return data_io
...
def parse_and_display_net(path, output):
    ...
    fig.tight_layout()
    plt.savefig(os.path.join(output, "logs-pics/fig-net.png"))
    plt.clf()
    return data_net
...
def parse_and_display_state(path, output):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output, "logs-pics/fig-state.png"), dpi=300)
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output, "logs-pics/fig-state-vert.png"), dpi=300)
    plt.clf()
    return data_state
